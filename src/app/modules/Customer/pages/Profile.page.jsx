...
function ProfilePage() {
    ...
    useEffect(() => {
        const getTrashed = async () => {
            try {
                const { data } = await CustomerService.trashed();
                setUser(data)
            } catch (error) {
                console.log(error)
            }
    ...
    const handlerDelete = async (id, user_name) => {
        ...
        const result = confirm(`Do you want delete ${user_name}`)
        if (result) {
            setLoading(true)
            setTimeout(() => {
    ...
    return (
        <DataTable
            ...
            metadata={metadata}
            toolbar={
                <Link to="/Customers/list" className="btn btn-success font-weight-bolder font-size-sm">
                    Back
                </Link>
            }
                ...
                <>
                    {user.map((data, index) => (
                        <tr key={index} data-row={1} className="datatable-row datatable-row-even" style={{ left: '0px' }}>
                            ...
                            <td data-field="name" aria-label="null" className="datatable-cell" width={`15%`}>
                                <img src={`https://6187ce0b057b9b00177f9ab3.mockapi.io/user/${user.image}`} alt="" />
                            </td>
